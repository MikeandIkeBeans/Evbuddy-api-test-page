...
from routes.cpms import _cpms_get_entry, _cpms_list_entries_by_asset
from src.infrastructure.persistence import SessionRepository
...
def ev_get_all_chargers():
    ...
    if status:
        url = f"{EVBUDDY_DEV_CHARGERS_BASE}/chargers/chargers/status/{status}"
    elif site_id:
        url = f"{EVBUDDY_DEV_CHARGERS_BASE}/chargers/chargers/site/{site_id}"
    else:
        url = f"{EVBUDDY_DEV_CHARGERS_BASE}/chargers/chargers/status/available"
...
def ev_list_sessions():
    limit = normalized_limit(default=100, maximum=1000)
    persisted = {s["sessionId"]: s for s in SESSION_REPOSITORY.list_all() if s.get("sessionId")}
    persisted.update(EV_SESSIONS)
    result = []
    for session_id, session in persisted.items():
    ...
    result.sort(key=lambda s: s.get("startedAt") or "", reverse=True)
    bounded = result[:limit]
    return jsonify({"sessions": bounded, "count": len(bounded), "total": len(result)})
...
def ev_create_session():
    ...
    session = {
        ...
        "_idTag": "HOTEL-GUEST",
        "_useRealCharger": True,
    }

    EV_SESSIONS[session_id] = session
    SESSION_REPOSITORY.upsert(session)
...
def ev_get_session(session_id):
    session = SESSION_REPOSITORY.get(session_id) or EV_SESSIONS.get(session_id)
    if not session:
...
@ev_charging_bp.route("/v1/debug/sessions/<session_id>/correlation", methods=["GET"])
def ev_session_correlation(session_id):
    """Correlate local session state with CPMS transaction records for debugging."""
    session = SESSION_REPOSITORY.get(session_id) or EV_SESSIONS.get(session_id)
    if not session:
        return Response("Unknown session", status=404)
    ...
    s = ev_recompute_session(session)
    cpms_match = None
    txid = s.get("_transactionId")

    if txid is not None:
        cpms_match = _cpms_get_entry(str(txid))

    asset_commands = _cpms_list_entries_by_asset(s["chargerId"], limit=10)
    ...
    return jsonify(
        {
            "session": {
                ...
                "energyKwh": s["energyKwh"],
                "transactionId": txid,
                "stopRequested": s.get("stopRequested", False),
                "startedAt": s.get("startedAt"),
                "endedAt": s.get("endedAt"),
            },
            "cpms": {
                "transactionMatch": cpms_match,
                "recentAssetCommands": asset_commands,
            },
        }
    )
...
def ev_health_check():
    health = {
        "status": "ok",
        "mode": "dev",
        "devApiBase": EVBUDDY_DEV_USERS_BASE,
        "devApiConnected": False,
        "useDevCharger": True,
        # Legacy keys retained for existing clients.
        "realApiBase": EVBUDDY_DEV_USERS_BASE,
        "realApiConnected": False,
        "useRealCharger": True,
    }

    try:
        ev_call_dev_api("/user/getByEmail", {"email": "test@test.com"}, "POST")
        health["devApiConnected"] = True
        ...
        if "404" in error_msg or "not found" in error_msg.lower():
            ...
            health["devApiConnected"] = False
            health["realApiConnected"] = False
            health["devApiError"] = error_msg
            health["realApiError"] = error_msg

    return jsonify(health)


@ev_charging_bp.route("/v1/dev/user/getByEmail", methods=["POST"])
@ev_charging_bp.route("/v1/real/user/getByEmail", methods=["POST"])
def ev_dev_get_user_by_email():
    data = request.get_json() or {}
    email = data.get("email")
    if not email:
    ...
    try:
        user = ev_get_dev_user_by_email(email)
        if not user:
...
@ev_charging_bp.route("/v1/dev/users", methods=["GET"])
@ev_charging_bp.route("/v1/real/users", methods=["GET"])
def ev_dev_get_users():
    try:
        users = ev_get_dev_users()
        return jsonify(users)
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@ev_charging_bp.route("/v1/dev/proxy", methods=["POST"])
...
def ev_dev_proxy():
    ...
    try:
        result = ev_call_dev_api(endpoint, body or {})
...
def ev_get_host_sites():
    url = f"{EVBUDDY_DEV_HOST_SITES_BASE}/host-sites"
    return ev_simple_get(url)


@ev_charging_bp.route("/v1/host-sites/<int:site_id>", methods=["GET"])
def ev_get_host_site(site_id):
    url = f"{EVBUDDY_DEV_HOST_SITES_BASE}/host-sites/{site_id}"
    return ev_simple_get(url)
...
def ev_get_charger_details(charger_id):
    url = f"{EVBUDDY_DEV_CHARGERS_BASE}/chargers/chargers/{charger_id}"
    return ev_simple_get(url)
...
def ev_get_charger_ocpp_status(charge_point_id):
    try:
        ...
        cp_url = f"{EVBUDDY_DEV_OCPP_BASE}/api/charge-points"
...
def ev_get_charge_points():
    url = f"{EVBUDDY_DEV_OCPP_BASE}/api/charge-points"
...
def ev_get_ocpp_sessions():
    ...
    try:
        ...
        for cp in charge_points:
            ...
            connectors_url = f"{EVBUDDY_DEV_OCPP_BASE}/api/connectors"
